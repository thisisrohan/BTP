...
@annotate(
    double='pa_x, pa_y, pb_x, pb_y, pc_x, pc_y, splitter, ccwerrboundA, ' + \
           'ccwerrboundB, ccwerrboundC, resulterrbound, det, detsum, return_')
def orient2d(
        pa_x, pa_y, pb_x, pb_y, pc_x, pc_y, splitter, ccwerrboundA,
...
def incircleadapt(
    ...
    len(catt) = 4
    '''
    bc, ca, ab, aa, bb, cc, u, v, abtt, bctt, catt = declare(
        'matrix(4, "double")', 11)
    axbc, aybc, bxca, byca, cxab, cyab, temp8, axtbb, axtcc, aytbb, aytcc, \
    bxtaa, bxtcc, bytaa, bytcc, cxtaa, cxtbb, cytaa, cytbb, axtbc, aytbc, \
    bxtca, bytca, cxtab, cytab, axtbctt, aytbctt, bxtcatt, bytcatt, cxtabtt, \
    cytabtt, abt, bct, cat = declare('matrix(8, "double")', 34)
    axxbc, ayybc, bxxca, byyca, cxxab, cyyab, temp16a, temp16b, temp16c, \
    axtbct, aytbct, bxtcat, bytcat, cxtabt, cytabt = declare(
        'matrix(16, "double")', 15)
    adet, bdet, cdet, temp32a, temp32b = declare('matrix(32, "double")', 5)
    temp48 = declare('matrix(48, "double")')
    abdet, temp64 = declare('matrix(64, "double")', 2)
    fin1, fin2 = declare('matrix(1152, "double")', 2)

    ablen, abttlen, bcttlen, cattlen, axbclen, aybclen, bxcalen, bycalen, \
    cxablen, cyablen, temp8len, axtbblen, axtcclen, aytbblen, aytcclen, \
    bxtaalen, bxtcclen, bytaalen, bytcclen, cxtaalen, cxtbblen, cytaalen, \
    cytbblen, axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen, \
    axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen, \
    abtlen, bctlen, catlen, axxbclen, ayybclen, bxxcalen, byycalen, cxxablen, \
    cyyablen, temp16alen, temp16blen, temp16clen, axtbctlen, aytbctlen, \
    bxtcatlen, bytcatlen, cxtabtlen, cytabtlen, alen, blen, clen, temp32alen, \
    temp32blen, temp48len, abdetlen, temp64len, finlength = declare('int', 62)

    finnow, finother, finswap = declare('doublep', 3)

    adx, bdx, cdx, ady, bdy, cdy, bdxcdy1, bdxcdy0, cdxbdy1, cdxbdy0, \
    cdxady1, cdxady0, adxcdy1, adxcdy0, adxbdy1, adxbdy0, bdxady1, bdxady0, \
    det, errbound, adxtail, adytail, bdxtail, bdytail, cdxtail, cdytail, \
    adxadx1, adxadx0, adyady1, adyady0, bdxbdx1, bdxbdx0, bdybdy1, bdybdy0, \
    cdxcdx1, cdxcdx0, cdycdy1, cdycdy0, ti1, ti0, tj1, tj0, negate = declare(
        'double', 43)
...
@annotate(
    double='pa_x, pa_y, pb_x, pb_y, pc_x, pc_y, pd_x, pd_y, splitter, ' + \
           'iccerrboundA, iccerrboundB, iccerrboundC, resulterrbound, ' + \
           'det, permanent, return_')
def incircle(
        pa_x, pa_y, pb_x, pb_y, pc_x, pc_y, pd_x, pd_y, splitter, iccerrboundA,
