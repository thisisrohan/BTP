...
def initialize_seg_ht(seg_ht_cap, seg_ht_arr, segments, num_segs):
    ...
    # print("INITIALIZING SEGMENTS HASH TABLE")
    # print("seg_ht_cap : {}".format(seg_ht_cap))
    for i in range(num_segs):
        # print("-- seg_idx : {} --".format(i))
        v1 = segments[i, 0]
        v2 = segments[i, 1]
        key = (v1 + 1)*(v2 + 1)
        h_index = key % seg_ht_cap
        while True:
            # print("    h_index : {}".format(h_index))
            if seg_ht_arr[h_index, 0] < 0:
                ...
                if h_index >= seg_ht_cap:
                    h_index -= seg_ht_cap
        # print("    key : {}, seg_idx : {}, hindex : {}".format(key, i, h_index))
...
def get_seg_idx(v1, v2, seg_ht_cap, seg_ht_arr, segments):
    ...
    while True:
        ...
        if seg_ht_arr[h_index, 0] == -1:
            ...
            # print("    seg_idx : {}, v1_s : {}, v2_s : {}".format(seg_idx, v1_s, v2_s))
...
def delete_entry_tri_ht(v1, v2, v3, t_index, tri_ht_cap, tri_ht_arr):
    ...
    h_index1_org = h_index1
    while True:
        if tri_ht_arr[h_index1, 0] == key1 and \
                tri_ht_arr[h_index1, 1] == t_index:
            ...
            if h_index1 >= tri_ht_cap:
                h_index1 -= tri_ht_cap
            if h_index1 == h_index1_org:
                break

    # deleting in tri_ht_arr2
    h_index2_org = h_index2
    while True:
        if tri_ht_arr[h_index2, 2] == key2 and \
                tri_ht_arr[h_index2, 3] == t_index:
            ...
            if h_index2 >= tri_ht_cap:
                h_index2 -= tri_ht_cap
            if h_index2 == h_index2_org:
                break

    # deleting in tri_ht_arr3
    h_index3_org = h_index3
    while True:
        if tri_ht_arr[h_index3, 4] == key3 and \
                tri_ht_arr[h_index3, 5] == t_index:
...
def add_new_tri_to_tri_ht(
    ...
    if num_tri >= tri_ht_cap:
        tri_ht_cap *= 2
        tri_ht_arr = -1*np.ones(shape=(tri_ht_cap, 6), dtype=np.int64)
        initialize_tri_ht(
