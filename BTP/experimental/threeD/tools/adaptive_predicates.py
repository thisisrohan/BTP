...
@njit(cache=True)
def exactinit2d(points, exactinit_arr):
    ...
    exactinit_arr[1] = (3.0 + 8.0 * epsilon) * epsilon  # resulterrbound
    exactinit_arr[2] = (3.0 + 16.0 * epsilon) * epsilon  # ccwerrboundA
    exactinit_arr[3] = (2.0 + 12.0 * epsilon) * epsilon  # ccwerrboundB
    exactinit_arr[4] = (9.0 + 64.0 * epsilon) * epsilon * epsilon  # ccwerrboundC
    exactinit_arr[5] = (10.0 + 96.0 * epsilon) * epsilon  # iccerrboundA
    exactinit_arr[6] = (4.0 + 48.0 * epsilon) * epsilon  # iccerrboundB
    exactinit_arr[7] = (44.0 + 576.0 * epsilon) * epsilon * epsilon  # iccerrboundC
    exactinit_arr[8] = 32 * epsilon * b * b  # static_filter_o2d
    exactinit_arr[9] = 1984 * epsilon * b * b * b  # static_filter_i2d

    return


@njit(cache=True)
def exactinit3d(points, exactinit_arr):
    ...
    # Error bounds for orientation and incircle tests.
    exactinit_arr[1] = (3.0 + 8.0 * epsilon) * epsilon  # resulterrbound
    exactinit_arr[2] = (7.0 + 56.0 * epsilon) * epsilon  # o3derrboundA
    exactinit_arr[3] = (3.0 + 28.0 * epsilon) * epsilon  # o3derrboundB
    exactinit_arr[4] = (26.0 + 288.0 * epsilon) * epsilon * epsilon  # o3derrboundC
    exactinit_arr[5] = (16.0 + 224.0 * epsilon) * epsilon  # isperrboundA
    exactinit_arr[6] = (5.0 + 72.0 * epsilon) * epsilon  # isperrboundB
    exactinit_arr[7] = (71.0 + 1408.0 * epsilon) * epsilon * epsilon  # isperrboundC
    exactinit_arr[8] = 352 * epsilon * b * b * b  # static_filter_o3d
    exactinit_arr[9] = 33024 * epsilon * b * b * b * b  # static_filter_i3d
...
@njit(cache=True)
def orient3dadapt(
        pa_x, pa_y, pa_z, pb_x, pb_y, pb_z, pc_x, pc_y, pc_z, pd_x, pd_y, pd_z,
        permanent, global_arr, exactinit_arr):
    '''
    len(bc) = 4
    len(ca) = 4
    len(ab) = 4
    ...
    ca = global_arr[4:8]
    ab = global_arr[8:12]
    adet = global_arr[12:20]
    bdet = global_arr[20:28]
    cdet = global_arr[28:36]
    abdet = global_arr[36:52]
    fin1 = global_arr[52:244]
    fin2 = global_arr[244:436]
    at_b = global_arr[436:440]
    at_c = global_arr[440:444]
    bt_c = global_arr[444:448]
    bt_a = global_arr[448:452]
    ct_a = global_arr[452:456]
    ct_b = global_arr[456:460]
    bct = global_arr[460:468]
    cat = global_arr[468:476]
    abt = global_arr[476:484]
    u = global_arr[484:488]
    v = global_arr[488:490]
    w = global_arr[490:506]

    splitter = exactinit_arr[0]
    resulterrbound = exactinit_arr[1]
    o3derrboundA = exactinit_arr[2]
    o3derrboundB = exactinit_arr[3]
    o3derrboundC = exactinit_arr[4]
    ...
    adztail = Two_Diff_Tail(pa_z, pd_z, adz)
    bdztail = Two_Diff_Tail(pb_z, pd_z, bdz)
    cdztail = Two_Diff_Tail(pc_z, pd_z, cdz)
...
def orient3d(
    ...
    static_filter_o3d = exactinit_arr[8]
    o3derrboundA = exactinit_arr[2]
    ...
    if np.abs(det) > static_filter_o3d:
        return det
...
def insphereexact(
        pa_x, pa_y, pa_z, pb_x, pb_y, pb_z, pc_x, pc_y, pc_z, pd_x, pd_y, pd_z,
        pe_x, pe_y, pe_z, global_arr, exactinit_arr):
    '''
    len(ab) = 4
    len(bc) = 4
    len(cd) = 4
    ...
    da = global_arr[32:36]
    eb = global_arr[36:40]
    temp8a = global_arr[40:48]
    temp8b = global_arr[48:56]
    temp16 = global_arr[64:80]
    abc = global_arr[80:104]
    bcd = global_arr[104:128]
    cde = global_arr[128:152]
    dea = global_arr[152:176]
    eab = global_arr[176:200]
    abd = global_arr[200:224]
    bce = global_arr[224:248]
    cda = global_arr[248:272]
    deb = global_arr[272:296]
    eac = global_arr[296:320]
    temp48a = global_arr[320:368]
    temp48b = global_arr[368:416]
    abcd = global_arr[416:512]
    bcde = global_arr[512:608]
    cdea = global_arr[608:704]
    deab = global_arr[704:800]
    eabc = global_arr[800:896]
    det384x = global_arr[896:1280]
    det384y = global_arr[1280:1664]
    det384z = global_arr[1664:2048]
    detxy = global_arr[2048:2816]
    adet = global_arr[2816:3968]
    bdet = global_arr[3968:5120]
    cdet = global_arr[5120:6272]
    ddet = global_arr[6272:7424]
    edet = global_arr[7424:8576]
    abdet = global_arr[8576:10880]
    cddet = global_arr[10880:13184]
    cdedet = global_arr[13184:16640]
    deter = global_arr[16640:22400]
    temp192 = global_arr[22400:22592]

    splitter = exactinit_arr[0]

    axby1, axby0 = Two_Product(pa_x, pb_y, splitter)
    bxay1, bxay0 = Two_Product(pb_x, pa_y, splitter)
    ab[3], ab[2], ab[1], ab[0] = Two_Two_Diff(axby1, axby0, bxay1, bxay0)

    bxcy1, bxcy0 = Two_Product(pb_x, pc_y, splitter)
    cxby1, cxby0 = Two_Product(pc_x, pb_y, splitter)
    bc[3], bc[2], bc[1], bc[0] = Two_Two_Diff(bxcy1, bxcy0, cxby1, cxby0)

    cxdy1, cxdy0 = Two_Product(pc_x, pd_y, splitter)
    dxcy1, dxcy0 = Two_Product(pd_x, pc_y, splitter)
    cd[3], cd[2], cd[1], cd[0] = Two_Two_Diff(cxdy1, cxdy0, dxcy1, dxcy0)

    dxey1, dxey0 = Two_Product(pd_x, pe_y, splitter)
    exdy1, exdy0 = Two_Product(pe_x, pd_y, splitter)
    de[3], de[2], de[1], de[0] = Two_Two_Diff(dxey1, dxey0, exdy1, exdy0)

    exay1, exay0 = Two_Product(pe_x, pa_y, splitter)
    axey1, axey0 = Two_Product(pa_x, pe_y, splitter)
    ea[3], ea[2], ea[1], ea[0] = Two_Two_Diff(exay1, exay0, axey1, axey0)

    axcy1, axcy0 = Two_Product(pa_x, pc_y, splitter)
    cxay1, cxay0 = Two_Product(pc_x, pa_y, splitter)
    ac[3], ac[2], ac[1], ac[0] = Two_Two_Diff(axcy1, axcy0, cxay1, cxay0)

    bxdy1, bxdy0 = Two_Product(pb_x, pd_y, splitter)
    dxby1, dxby0 = Two_Product(pd_x, pb_y, splitter)
    bd[3], bd[2], bd[1], bd[0] = Two_Two_Diff(bxdy1, bxdy0, dxby1, dxby0)

    cxey1, cxey0 = Two_Product(pc_x, pe_y, splitter)
    excy1, excy0 = Two_Product(pe_x, pc_y, splitter)
    ce[3], ce[2], ce[1], ce[0] = Two_Two_Diff(cxey1, cxey0, excy1, excy0)

    dxay1, dxay0 = Two_Product(pd_x, pa_y, splitter)
    axdy1, axdy0 = Two_Product(pa_x, pd_y, splitter)
    da[3], da[2], da[1], da[0] = Two_Two_Diff(dxay1, dxay0, axdy1, axdy0)

    exby1, exby0 = Two_Product(pe_x, pb_y, splitter)
    bxey1, bxey0 = Two_Product(pb_x, pe_y, splitter)
    eb[3], eb[2], eb[1], eb[0] = Two_Two_Diff(exby1, exby0, bxey1, bxey0)

    temp8alen = scale_expansion_zeroelim(4, bc, pa_z, temp8a, splitter)
    temp8blen = scale_expansion_zeroelim(4, ac, -pb_z, temp8b, splitter)
    temp16len = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp8blen,
                                            temp8b, temp16)
    temp8alen = scale_expansion_zeroelim(4, ab, pc_z, temp8a, splitter)
    abclen = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp16len, temp16,
                                         abc)

    temp8alen = scale_expansion_zeroelim(4, cd, pb_z, temp8a, splitter)
    temp8blen = scale_expansion_zeroelim(4, bd, -pc_z, temp8b, splitter)
    temp16len = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp8blen,
                                            temp8b, temp16)
    temp8alen = scale_expansion_zeroelim(4, bc, pd_z, temp8a, splitter)
    bcdlen = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp16len, temp16,
                                         bcd)

    temp8alen = scale_expansion_zeroelim(4, de, pc_z, temp8a, splitter)
    temp8blen = scale_expansion_zeroelim(4, ce, -pd_z, temp8b, splitter)
    temp16len = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp8blen,
                                            temp8b, temp16)
    temp8alen = scale_expansion_zeroelim(4, cd, pe_z, temp8a, splitter)
    cdelen = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp16len, temp16,
                                         cde)

    temp8alen = scale_expansion_zeroelim(4, ea, pd_z, temp8a, splitter)
    temp8blen = scale_expansion_zeroelim(4, da, -pe_z, temp8b, splitter)
    temp16len = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp8blen,
                                            temp8b, temp16)
    temp8alen = scale_expansion_zeroelim(4, de, pa_z, temp8a, splitter)
    dealen = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp16len, temp16,
                                         dea)

    temp8alen = scale_expansion_zeroelim(4, ab, pe_z, temp8a, splitter)
    temp8blen = scale_expansion_zeroelim(4, eb, -pa_z, temp8b, splitter)
    temp16len = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp8blen,
                                            temp8b, temp16)
    temp8alen = scale_expansion_zeroelim(4, ea, pb_z, temp8a, splitter)
    eablen = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp16len, temp16,
                                         eab)

    temp8alen = scale_expansion_zeroelim(4, bd, pa_z, temp8a, splitter)
    temp8blen = scale_expansion_zeroelim(4, da, pb_z, temp8b, splitter)
    temp16len = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp8blen,
                                            temp8b, temp16)
    temp8alen = scale_expansion_zeroelim(4, ab, pd_z, temp8a, splitter)
    abdlen = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp16len, temp16,
                                         abd)

    temp8alen = scale_expansion_zeroelim(4, ce, pb_z, temp8a, splitter)
    temp8blen = scale_expansion_zeroelim(4, eb, pc_z, temp8b, splitter)
    temp16len = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp8blen,
                                            temp8b, temp16)
    temp8alen = scale_expansion_zeroelim(4, bc, pe_z, temp8a, splitter)
    bcelen = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp16len, temp16,
                                         bce)

    temp8alen = scale_expansion_zeroelim(4, da, pc_z, temp8a, splitter)
    temp8blen = scale_expansion_zeroelim(4, ac, pd_z, temp8b, splitter)
    temp16len = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp8blen,
                                            temp8b, temp16)
    temp8alen = scale_expansion_zeroelim(4, cd, pa_z, temp8a, splitter)
    cdalen = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp16len, temp16,
                                         cda)

    temp8alen = scale_expansion_zeroelim(4, eb, pd_z, temp8a, splitter)
    temp8blen = scale_expansion_zeroelim(4, bd, pe_z, temp8b, splitter)
    temp16len = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp8blen,
                                            temp8b, temp16)
    temp8alen = scale_expansion_zeroelim(4, de, pb_z, temp8a, splitter)
    deblen = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp16len, temp16,
                                         deb)

    temp8alen = scale_expansion_zeroelim(4, ac, pe_z, temp8a, splitter)
    temp8blen = scale_expansion_zeroelim(4, ce, pa_z, temp8b, splitter)
    temp16len = fast_expansion_sum_zeroelim(temp8alen, temp8a, temp8blen,
                                            temp8b, temp16)
    temp8alen = scale_expansion_zeroelim(4, ea, pc_z, temp8a, splitter)
    ...
    bcdelen = fast_expansion_sum_zeroelim(temp48alen, temp48a, temp48blen,
                                          temp48b, bcde)
    xlen = scale_expansion_zeroelim(bcdelen, bcde, pa_x, temp192, splitter)
    xlen = scale_expansion_zeroelim(xlen, temp192, pa_x, det384x, splitter)
    ylen = scale_expansion_zeroelim(bcdelen, bcde, pa_y, temp192, splitter)
    ylen = scale_expansion_zeroelim(ylen, temp192, pa_y, det384y, splitter)
    zlen = scale_expansion_zeroelim(bcdelen, bcde, pa_z, temp192, splitter)
    zlen = scale_expansion_zeroelim(zlen, temp192, pa_z, det384z, splitter)
    ...
    cdealen = fast_expansion_sum_zeroelim(temp48alen, temp48a, temp48blen,
                                          temp48b, cdea)
    xlen = scale_expansion_zeroelim(cdealen, cdea, pb_x, temp192, splitter)
    xlen = scale_expansion_zeroelim(xlen, temp192, pb_x, det384x, splitter)
    ylen = scale_expansion_zeroelim(cdealen, cdea, pb_y, temp192, splitter)
    ylen = scale_expansion_zeroelim(ylen, temp192, pb_y, det384y, splitter)
    zlen = scale_expansion_zeroelim(cdealen, cdea, pb_z, temp192, splitter)
    zlen = scale_expansion_zeroelim(zlen, temp192, pb_z, det384z, splitter)
    xylen = fast_expansion_sum_zeroelim(xlen, det384x, ylen, det384y, detxy)
    ...
    deablen = fast_expansion_sum_zeroelim(temp48alen, temp48a, temp48blen,
                                          temp48b, deab)
    xlen = scale_expansion_zeroelim(deablen, deab, pc_x, temp192, splitter)
    xlen = scale_expansion_zeroelim(xlen, temp192, pc_x, det384x, splitter)
    ylen = scale_expansion_zeroelim(deablen, deab, pc_y, temp192, splitter)
    ylen = scale_expansion_zeroelim(ylen, temp192, pc_y, det384y, splitter)
    zlen = scale_expansion_zeroelim(deablen, deab, pc_z, temp192, splitter)
    zlen = scale_expansion_zeroelim(zlen, temp192, pc_z, det384z, splitter)
    xylen = fast_expansion_sum_zeroelim(xlen, det384x, ylen, det384y, detxy)
    ...
    eabclen = fast_expansion_sum_zeroelim(temp48alen, temp48a, temp48blen,
                                          temp48b, eabc)
    xlen = scale_expansion_zeroelim(eabclen, eabc, pd_x, temp192, splitter)
    xlen = scale_expansion_zeroelim(xlen, temp192, pd_x, det384x, splitter)
    ylen = scale_expansion_zeroelim(eabclen, eabc, pd_y, temp192, splitter)
    ylen = scale_expansion_zeroelim(ylen, temp192, pd_y, det384y, splitter)
    zlen = scale_expansion_zeroelim(eabclen, eabc, pd_z, temp192, splitter)
    zlen = scale_expansion_zeroelim(zlen, temp192, pd_z, det384z, splitter)
    xylen = fast_expansion_sum_zeroelim(xlen, det384x, ylen, det384y, detxy)
    ...
    abcdlen = fast_expansion_sum_zeroelim(temp48alen, temp48a, temp48blen,
                                          temp48b, abcd)
    xlen = scale_expansion_zeroelim(abcdlen, abcd, pe_x, temp192, splitter)
    xlen = scale_expansion_zeroelim(xlen, temp192, pe_x, det384x, splitter)
    ylen = scale_expansion_zeroelim(abcdlen, abcd, pe_y, temp192, splitter)
    ylen = scale_expansion_zeroelim(ylen, temp192, pe_y, det384y, splitter)
    zlen = scale_expansion_zeroelim(abcdlen, abcd, pe_z, temp192, splitter)
    zlen = scale_expansion_zeroelim(zlen, temp192, pe_z, det384z, splitter)
    xylen = fast_expansion_sum_zeroelim(xlen, det384x, ylen, det384y, detxy)
...
def insphereadapt(
        pa_x, pa_y, pa_z, pb_x, pb_y, pb_z, pc_x, pc_y, pc_z, pd_x, pd_y, pd_z,
        pe_x, pe_y, pe_z, permanent, global_arr, exactinit_arr):
    '''
    len(ab) = 4
    len(bc) = 4
    len(cd) = 4
    ...
    temp8a = global_arr[40:48]
    temp8b = global_arr[48:56]
    temp8c = global_arr[56:64]
    temp16 = global_arr[64:80]
    temp24 = global_arr[80:104]
    temp48 = global_arr[104:152]
    xdet = global_arr[152:248]
    ydet = global_arr[248:344]
    zdet = global_arr[344:440]
    xydet = global_arr[440:632]
    adet = global_arr[632:920]
    bdet = global_arr[920:1208]
    cdet = global_arr[1208:1496]
    ddet = global_arr[1496:1784]
    abdet = global_arr[1784:2360]
    cddet = global_arr[2360:2936]
    fin1 = global_arr[2936:4088]

    splitter = exactinit_arr[0]
    resulterrbound = exactinit_arr[1]
    isperrboundA = exactinit_arr[5]
    isperrboundB = exactinit_arr[6]
    isperrboundC = exactinit_arr[7]
    ...
    aextail = Two_Diff_Tail(pa_x, pe_x, aex)
    aeytail = Two_Diff_Tail(pa_y, pe_y, aey)
    aeztail = Two_Diff_Tail(pa_z, pe_z, aez)
    bextail = Two_Diff_Tail(pb_x, pe_x, bex)
    beytail = Two_Diff_Tail(pb_y, pe_y, bey)
    beztail = Two_Diff_Tail(pb_z, pe_z, bez)
    cextail = Two_Diff_Tail(pc_x, pe_x, cex)
    ceytail = Two_Diff_Tail(pc_y, pe_y, cey)
    ceztail = Two_Diff_Tail(pc_z, pe_z, cez)
    dextail = Two_Diff_Tail(pd_x, pe_x, dex)
    deytail = Two_Diff_Tail(pd_y, pe_y, dey)
    deztail = Two_Diff_Tail(pd_z, pe_z, dez)
...
def insphere(
    ...
    # commented this out because already computing a first approximation with
    # sub-determinants and checking if that is > static_filter_i3d
