...
REAL static_filter_o2d, static_filter_ic;
REAL static_filter_o3d, static_filter_is;
...
void exactinit2d(REAL* points, long num_points)
{
  REAL half, max_x, min_x, max_y, min_y, b;
  ...
  for ( i = 1; i < num_points; i++ ) {
	if ( points[2*i + 0] > max_x ) {
		max_x = points[2*i + 0];
	} else if ( points[2*i + 0] < min_x ) {
		min_x = points[2*i + 0];
	}

	if ( points[2*i + 1] > max_y ) {
		max_y = points[2*i + 1];
	} else if ( points[2*i + 1] < min_y ) {
		min_y = points[2*i + 1];
	}
  }
  ...
  static_filter_ic = 1984 * epsilon * b * b * b;
  return;
}
...
REAL orient2dadapt(pa, pb, pc, detsum, global_arr)
REAL *pa;
REAL *pb;
REAL *pc;
REAL detsum;
...
REAL orient2d(pa, pb, pc, global_arr)
REAL *pa;
REAL *pb;
REAL *pc;
double* global_arr;
{
  ...
  if ((det >= static_filter_o2d) || (-det >= static_filter_o2d)) {
	  return det;
  }
  ...
  return orient2dadapt(pa, pb, pc, detsum, global_arr);
}
...
REAL incircleadapt(pa, pb, pc, pd, permanent, global_arr)
REAL *pa;
REAL *pb;
REAL *pc;
REAL *pd;
...
REAL *global_arr;
{
  ...
  REAL *bc, *ca, *ab;
  INEXACT REAL bc3, ca3, ab3;
  REAL *axbc, *axxbc, *aybc, *ayybc, *adet;
  int axbclen, axxbclen, aybclen, ayybclen, alen;
  REAL *bxca, *bxxca, *byca, *byyca, *bdet;
  int bxcalen, bxxcalen, bycalen, byycalen, blen;
  REAL *cxab, *cxxab, *cyab, *cyyab, *cdet;
  ...
  REAL *u, *v;
  INEXACT REAL u3, v3;
  REAL *temp8, *temp16a, *temp16b, *temp16c;
  REAL *temp32a, *temp32b, *temp48, *temp64;
  int temp8len, temp16alen, temp16blen, temp16clen;
  int temp32alen, temp32blen, temp48len, temp64len;
  REAL *axtbb, *axtcc, *aytbb, *aytcc;
  int axtbblen, axtcclen, aytbblen, aytcclen;
  REAL *bxtaa, *bxtcc, *bytaa, *bytcc;
  int bxtaalen, bxtcclen, bytaalen, bytcclen;
  REAL *cxtaa, *cxtbb, *cytaa, *cytbb;
  int cxtaalen, cxtbblen, cytaalen, cytbblen;
  REAL *axtbc, *aytbc, *bxtca, *bytca, *cxtab, *cytab;
  int axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
  REAL *axtbct, *aytbct, *bxtcat, *bytcat, *cxtabt, *cytabt;
  int axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
  REAL *axtbctt, *aytbctt, *bxtcatt;
  REAL *bytcatt, *cxtabtt, *cytabtt;
  ...
  adet = &global_arr[108];
  bxca = &global_arr[140];
  bxxca = &global_arr[148];
  byca = &global_arr[164];
  byyca = &global_arr[172];
  bdet = &global_arr[188];
  cxab = &global_arr[220];
  cxxab = &global_arr[228];
  cyab = &global_arr[244];
  cyyab = &global_arr[252];
  cdet = &global_arr[268];
  abdet = &global_arr[300];
  fin1 = &global_arr[364];
  fin2 = &global_arr[1516];
  aa = &global_arr[2668];
  bb = &global_arr[2672];
  cc = &global_arr[2676];
  temp8 = &global_arr[2680];
  temp16a = &global_arr[2688];
  temp16b = &global_arr[2704];
  temp16c = &global_arr[2720];
  temp32a = &global_arr[2736];
  temp32b = &global_arr[2768];
  temp48 = &global_arr[2800];
  temp64 = &global_arr[2848];
  axtbb = &global_arr[2912];
  axtcc = &global_arr[2920];
  aytbb = &global_arr[2928];
  aytcc = &global_arr[2936];
  bxtaa = &global_arr[2944];
  bxtcc = &global_arr[2952];
  bytaa = &global_arr[2960];
  bytcc = &global_arr[2968];
  cxtaa = &global_arr[2976];
  cxtbb = &global_arr[2984];
  cytaa = &global_arr[2992];
  cytbb = &global_arr[3000];
  axtbc = &global_arr[3008];
  aytbc = &global_arr[3016];
  bxtca = &global_arr[3024];
  bytca = &global_arr[3032];
  cxtab = &global_arr[3040];
  cytab = &global_arr[3048];
  axtbct = &global_arr[3056];
  aytbct = &global_arr[3072];
  bxtcat = &global_arr[3088];
  bytcat = &global_arr[3104];
  cxtabt = &global_arr[3120];
  cytabt = &global_arr[3136];
  axtbctt = &global_arr[3152];
  aytbctt = &global_arr[3160];
  bxtcatt = &global_arr[3168];
  bytcatt = &global_arr[3176];
  cxtabtt = &global_arr[3184];
  cytabtt = &global_arr[3192];
  abt = &global_arr[3200];
  bct = &global_arr[3208];
  cat = &global_arr[3216];
  abtt = &global_arr[3224];
  bctt = &global_arr[3228];
  catt = &global_arr[3232];
...
REAL incircle(pa, pb, pc, pd, global_arr)
REAL *pa;
REAL *pb;
REAL *pc;
REAL *pd;
double* global_arr;
{
  ...
  det = alift * (bdxcdy - cdxbdy)
	...
	if ((det >= static_filter_ic) || (-det >= static_filter_ic)) {
		return det;
	}
  ...
  return incircleadapt(pa, pb, pc, pd, permanent, global_arr);
}
...
double find_intersection(double* pa, double* pb, double* pc) {

  double ab, ab_tail, cb, cb_tail, ca, ca_tail, res;
  INEXACT REAL bvirt;
  REAL avirt, bround, around;

  Two_Diff(pa[0], pb[0], ab, ab_tail);
  if ( ( ab == 0.0 ) && ( ab_tail == 0.0 ) ) {
    // use y co-ordinates
    Two_Diff(pa[1], pb[1], ab, ab_tail);
    Two_Diff(pc[1], pb[1], cb, cb_tail);
    Two_Diff(pc[1], pa[1], ca, ca_tail);
  } else {
    // use x co-ordinates
    Two_Diff(pc[0], pb[0], cb, cb_tail);
    Two_Diff(pc[0], pa[0], ca, ca_tail);
  }

  if ( ( ab < 0.0 ) || ( ab == 0.0 && ab_tail < 0.0 ) ) {
    // checking if alpha < 1.0
    if ( ( ca > 0.0 ) || ( ca == 0.0 && ca_tail > 0.0 ) ) {
      res = 0.5;
    } else {
      res = 1.5;
      return res;
    ...
    // checking if alpha > 0.0
    if ( ( cb < 0.0 ) || ( cb == 0.0 && cb_tail < 0.0 ) ) {
      return res;
    } else {
      res = -0.5;
      return res;
