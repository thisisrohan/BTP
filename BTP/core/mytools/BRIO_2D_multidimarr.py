...
def make_BRIO(points):
    ...
    org_points = np.empty(shape=(max_number_of_points_in_a_round, 2),
                          dtype=np.float64)
    temp_points = np.empty(shape=(max_number_of_points_in_a_round, 2),
                           dtype=np.float64)
    new_indices = np.empty(max_number_of_points_in_a_round, dtype=np.int64)
    new_points = np.empty(shape=(len_points, 2), dtype=np.float64)
...
def perf(N):
    ...
    temp = np.random.rand(20).reshape(10, 2)
